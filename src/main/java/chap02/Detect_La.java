...
public class Detect_La {
    ...
    if (args.length < 1) {
      ...
      out.printf(
          "\tjava %s \"<command>\" \"<path-to-image>\"\n"
              ...
              + "resource (gs://...)\n",
          Detect_La.class.getCanonicalName());
      return;
    }
    String command = args[0];
    String path = args.length > 1 ? args[1] : "";

    Detect_La app = new Detect_La(ImageAnnotatorClient.create());
  ...
  /**
   * Constructs a {@link Detect_La} which connects to the Cloud Vision API.
   *
   * @param client The Vision API client.
   */
  public Detect_La(ImageAnnotatorClient client) {
    visionApi = client;
  }
